...
NLP::LinguisticTree::LinguisticTree( const char* filename ){
    ...
    while( !strcmp( searcher.name(), "sentence" ) ){
        ...
        //cout<<parsing.first_child().value();
...
NLP::LinguisticTree::Node* NLP::LinguisticTree::Node::getPrev(){
    ...
    //otherwise return the parents previous child
...
NLP::LinguisticTree::Node* NLP::LinguisticTree::PhraseNode::getNext(){
    ...
    //if the node has no parents or children
    if( parent == NULL ) return NULL;
    //otherwise request the parent's next child
    return parent->getSelectChild( parentIndex+1 );
}

NLP::LinguisticTree::Node* NLP::LinguisticTree::PhraseNode::getSelectChild( int index ){
    //if the requested index is too small
    if( index < 0 ) return this;
    //if the requested iterator is too large
    if( (unsigned int) index >= children.size() ){
        //check if it has a parent
        if( parent == NULL ) return NULL;
        //then get the parents next child
        return parent->getSelectChild( parentIndex+1 );
    }
...
NLP::LinguisticTree::Iterator::Iterator():node(NULL){}

NLP::LinguisticTree::Iterator::Iterator( const Iterator& reference ):node( reference.node ){}
